...
import gdklsfcEnd from './gdklsf_end'
export default {
  components: {
    oddsButton,
    gdklsfcEnd
  ...
  computed: {
    ...
    setTabs () {
      return Number(this.$route.query.number_class) === 601
    }
  },
  methods: {
    // 计算按钮大小的样式
    setSize (index, key) {
      if (Number(this.$route.query.number_class) === 601) {
        return 'middle'
      }
      return 'small'
    },
    ...
    setClass (index) {
      ...
      if (Number(this.$route.query.number_class) === 602) {
        return 'is-full-100'
      }
      return 'is-full-25'
    },
