...
export default {
  ...
  methods: {
    setSize (index, key) {
      if (Number(this.$route.query.number_class) === 103) {
        if (key === 0 || key === 1) {
          return 'xLarge'
        }
      }
      return (Number(this.$route.query.number_class) === 101) && index === '1' ? 'middle' : 'small'
    },
    btnClickHandle (item) {
      console.log(item)
    ...
    setGroupClass (index) {
      return (Number(this.$route.query.number_class) === 101 || Number(this.$route.query.number_class) === 103) && index === '1' ? 'is-horizontal' : ''
    },
    setClass (index) {
      return Number(this.$route.query.number_class) === 101 && index === '1' ? 'is-full-100' : 'is-full-20'
    }
  },
  created () {
